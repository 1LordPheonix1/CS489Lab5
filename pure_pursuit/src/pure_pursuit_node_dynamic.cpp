...
public:
    PurePursuit() : Node("pure_pursuit_node"){
        ...
        currLookAhead = lookahead_distance;
    ...
    void pose_callback(const nav_msgs::msg::Odometry::SharedPtr pose_msg)
    {
        ...
        double min_speed = 0.3 * speed;

        double sp = speed - (std::abs(steering_angle) / deg_to_rad(max_steering_angle)) * (speed - min_speed);
        ...
        double min_lookAhead = 0.7 * lookahead_distance;

        currLookAhead = std::max(((sp / speed) * lookahead_distance), min_lookAhead);
